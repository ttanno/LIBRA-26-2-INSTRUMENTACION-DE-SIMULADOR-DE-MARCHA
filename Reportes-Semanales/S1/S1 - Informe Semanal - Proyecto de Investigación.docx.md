...
1. **Revisión y ampliación del estado del arte.** Revisión bibliográfica amplia en formato IEEE, cubriendo los temas clave del proyecto: simuladores de marcha con sensórica embebida, validación y calibración de IMU de bajo costo, celdas de carga en prótesis, protocolos de calibración robótica/mecatrónica, normas técnicas de ensayo de prótesis, arquitecturas de software de sincronización multi-sensor y diseño mecánico de plataformas móviles.

2. **Síntesis frente a objetivos del proyecto.** Tabla que cruza lo encontrado en el estado del arte con cada objetivo específico del proyecto. Brecha central identificada: ningún sistema revisado integra plataforma móvil + sensórica (IMU y fuerza) + software sincronizado + validación normativa en una sola arquitectura de bajo costo.

3. **Requerimientos técnicos preliminares.** Especificaciones orientativas para las áreas del proyecto (grados de libertad, tipo de IMU, rango de fuerza de la celda de carga, arquitectura de software, protocolo de validación), incluyendo la aclaración de alcance del sensado (continuo, sin lazo de control en tiempo real).

4. **Verificación normativa (ISO 10328).** Se verificaron, contra fuentes primarias, los rangos de fuerza y resistencia que la norma exige según el peso simulado del usuario — estos valores son los que en la práctica limitan qué tan robusta debe ser la celda de carga que se seleccione. Algunos niveles de carga quedaron confirmados con buena confianza; otros, correspondientes a los extremos de peso menos comunes, siguen sin verificar. Recomendación adoptada: dimensionar la celda de carga sobre un nivel intermedio-alto que cubra el rango de usuarios más probable, sin sobredimensionar para casos poco frecuentes.

5. **Consolidación de referencias bibliográficas.** Se corrigió la mayoría de las referencias que estaban incompletas, verificando autor, año, revista y DOI contra las fuentes originales; quedan algunas pendientes por no estar indexadas o no tener autor identificable. También se guardaron notas de texto completo de fuentes adicionales relevantes sobre celdas de carga, arquitecturas de software embebido y ensayos normativos sobre un pie protésico real.
