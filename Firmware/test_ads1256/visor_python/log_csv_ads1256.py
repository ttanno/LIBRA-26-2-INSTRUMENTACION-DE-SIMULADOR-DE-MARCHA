"""
Registro a CSV por cable (COM) de la celda de carga (test_ads1256.ino) --
sin graficos, pensado para juntar datos de una sesion de pesaje y
analizarlos despues en Excel/pandas. Es lo mismo que lee visor_ads1256.py,
pero en vez de graficar en vivo, guarda cada lectura a un archivo.

Uso:
    python log_csv_ads1256.py --port COM7
    python log_csv_ads1256.py --port COM7 --duracion 30
    python log_csv_ads1256.py --port COM7 --duracion 30 --salida prueba1.csv

Para grabar varias veces seguidas sin reescribir el comando cada vez (util
para repetir la prueba con distintos pesos):
    python log_csv_ads1256.py --port COM7 --duracion 15 --repeticiones 5
...
(ads1256_<fecha>_<hora>_repNdeM.csv) y al final se imprime una tabla
...
Este script NO tara ni calibra por vos -- antes de grabar, asegu/rate de
haber hecho 't' y 'k<masa>' en el Monitor Serie (o en visor_ads1256.py) al
...
crudos (raw_code/voltage_V siguen siendo validos aunque force_N no lo sea).
...
COLUMNAS = ["t_s", "raw_code", "voltage_V", "force_N", "tared", "calibrated"]
...
def grabar_una_vez(ser: serial.Serial, nombre_salida: str, duracion):
    ...
    try:
        with open(nombre_salida, "w", newline="", encoding="utf-8") as f:
            ...
            while True:
                ...
                if not m:
                    continue  # mensajes de arranque, confirmaciones de tara/calibracion, errores de DRDY, etc.
                ...
                if d["calibrated"] == "0" and not aviso_no_calibrado_mostrado:
                    print(
                        ...
                        "Se sigue registrando raw_code/voltage_V, pero force_N no es valido."
                    )
                ...
                writer.writerow([
                    f"{t:.3f}", d["raw_code"], d["voltage_V"], force_n,
    ...
    if filas >= 2:
        ...
        print("Muy pocas filas para calcular estadisticas -- revisa que el ADS1256 este enviando datos.")

    return resultado


def main():
    ap = argparse.ArgumentParser(description="Registro a CSV de la celda de carga (test_ads1256.ino) por cable (COM)")
    ...
    ap.add_argument(
        "--salida", default=None,
        help="Nombre base del archivo CSV de salida (default: ads1256_YYYYMMDD_HHMMSS.csv)",
    )
    ap.add_argument(
    ...
    base = args.salida or time.strftime("ads1256_%Y%m%d_%H%M%S")
