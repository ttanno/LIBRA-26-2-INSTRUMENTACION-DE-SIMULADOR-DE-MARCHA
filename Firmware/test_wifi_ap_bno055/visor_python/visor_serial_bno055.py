"""
Visor en tiempo real por CABLE (puerto serial / COM), para probar el
BNO055 SIN meter WiFi todavia -- funciona con el sketch que ya tengas
cargado ahora mismo, sea test_wifi_ap_bno055.ino o test_wifi_udp_bno055.ino,
porque los dos imprimen exactamente la misma linea por Serial como respaldo
de depuracion (ademas de mandarla por WiFi). No hace falta volver a cargar
ningun sketch ni tocar la parte de WiFi para usar esto.

Es lo mismo que visor_imu.py (el de test_bno055.ino con Adafruit) pero sin
el campo de temperatura, porque la libreria DFRobot_BNO055 no lo expone.

Uso:
    python visor_serial_bno055.py                  -> lista los puertos disponibles y pide elegir uno
    python visor_serial_bno055.py --port COM7
    python visor_serial_bno055.py --port COM7 --baud 115200

Botones:
    "Zero grafico (local)"   -> offset solo en este script (RAM), no se
                                guarda en el ESP32.
    "Cero absoluto (flash)"  -> manda 'z' por el cable -- promedia la
                                orientacion actual (manten el sensor quieto
                                ~1 s) y la guarda en flash del ESP32.
    "Borrar cero (flash)"    -> manda 'c' por el cable para borrar el cero
                                guardado.
...
def main():
    ap = argparse.ArgumentParser(description="Visor por cable (COM) para BNO055 -- sirve con cualquiera de los sketches de WiFi ya cargados")
    ...
    fig.suptitle("BNO055 (por cable, COM) - esperando datos...", y=0.99)
    ...
    def actualizar(_frame):
        ...
        try:
            while True:
                ...
                fig.suptitle(f"BNO055 (por cable, COM)   |   Calib sys={sys_c} gyro={gyro_c} accel={accel_c} mag={mag_c}")
                actualizado = True
        except Empty:
            pass
